...
class DatePicker(urwid.WidgetWrap):
    ...
    def __init__(self, initial_date=datetime.date.today(), *, date_range=RANGE.ALL, month_names=calendar.month_name, day_names=calendar.day_abbr,
        ...
        assert (type(date_range) == DatePicker.RANGE), DatePicker._TYPE_ERR_MSG.format("<enum 'DatePicker.RANGE'>",
                                                                                       "'date_range'",
                                                                                       type(date_range))
        
        for df in day_format:
            assert (type(df) == DatePicker.DAY_FORMAT), DatePicker._TYPE_ERR_MSG.format("<enum 'DatePicker.DAY_FORMAT'>",
        ...
        if date_range == DatePicker.RANGE.ALL:
            ...
            raise ValueError(DatePicker._VALUE_ERR_MSG.format(date_range))
        ...
        for day in range(start, end+1):
            ...
            for df in self._day_format:
                if df == DatePicker.DAY_FORMAT.DAY_OF_MONTH:
                    ...
                    raise ValueError(DatePicker._VALUE_ERR_MSG.format(df))
